    ...
    public:
        ...
        virtual void compute_unique_id(const int64_t id){
            std::mutex thread_lock;
                ...
                std::cout<<"UniqueID is "<<post_id<<std::endl;
        }
};
...
int main(){
    auto handler = make_shared<UniqueIDAsyncHandler>();
    auto proc = make_shared<UniqueIDAsyncProcessor>(handler);
    auto trans_svr = make_shared<TServerSocket>(3078);
