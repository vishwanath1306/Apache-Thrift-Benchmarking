...
class UniqueIDAsyncHandlerFactory: public UniqueIDAsyncIfFactory{
    ...
    public:
       ...
       virtual UniqueIDAsyncIf* getHandler(const TConnectionInfo& conn){
...
int main(){
    ...
    auto trans = make_shared<TNonblockingServerSocket>(3077);
    auto handler_fac = make_shared<UniqueIDAsyncHandlerFactory>();
    auto processor_fac = make_shared<UniqueIDAsyncProcessorFactory>(handler_fac);
    auto protocol_fac = make_shared<TCompactProtocolFactoryT<TMemoryBuffer>>();

    auto thread_man = ThreadManager::newSimpleThreadManager(8);
